...
# Load the saved AdaBoost classifier model
...
while True:
    ...
    for face in faces_rects:
        ...
        if embeddings:
            # Compare the face embedding with the ones from the training set using AdaBoost
            embedding = embeddings[0]  # Assuming one face per frame
            
            # Predict the class using the AdaBoost model
            predicted_class = adaboost.predict([embedding])
            confidence = adaboost.predict_proba([embedding]).max()  # Get the highest probability as confidence

            # Retrieve the name of the closest match
            predicted_name = person_names[int(predicted_class[0])]
